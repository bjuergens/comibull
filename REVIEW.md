...
## DB schema changes (frontend/src/store.ts)

* `DB_VERSION` bumped and a `migrations[N]` entry added?
* Incremental migration: data actually preserved? Test added?
* Migration getting big or touching multiple stores? Push back — `wipe` is fine for a prototype.
* Wipe: one-line `reason` on why incremental wasn't worth it?
* No backwards-compat shims leaked into callers (no `if (row.oldField)` etc.).
